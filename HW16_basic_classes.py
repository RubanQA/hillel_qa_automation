...
class Company:
    ...
    def get_avg_income(cls, *income: list):
        summ = sum(income[0])
        avg = round(summ / Company.compAmount, 2)
...
class Employee:
    ...
    def assigned_task(specialty):
        ...
        if specialty == 'Dev':
            ...
            requirement = ['qa_task_1', 'qa_task_2', 'qa_task_3']
        elif specialty == 'PM':
            requirement = ['pm_task_1', 'pm_task_2', 'pm_task_3']
        else:
...
epam = Company("Epam", 205, "Ukraine", 1000, 12700000)
companies = [itrex, epam]
rates = [company.income for company in companies]

itrex.view_conpany_info()
itrex.get_avg_income()
itrex.get_avg_salary()
